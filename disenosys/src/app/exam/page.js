...
export default function AdminPage() {
  ...
  const isExamOpen = (currentHour === 11 && currentMinute >= 0 && currentMinute <= 30);
      ...
      {isExamOpen ? (
        ...
        <div className="flex flex-col items-center justify-center w-full h-full p-5">
          <div className='flex flex-row space-x-8'>
            <div className="bg-[#182073] text-white text-lg p-10 rounded-lg shadow-lg max-w-md text-center space-y-6 rounded-b-lg border-b-8 border-red-600">
              <h2 className="font-bold text-2xl text-red-600">You Missed the Exam!</h2>
              <div className='flex justify-center items-center'>
                <TbMoodCry size={60} className='text-red-300' />
              </div>
              <p className="mt-4 text-base text-gray-300">
                The exam is open only from 11:00 AM to 11:30 AM. Please check the schedule for future exams.
              </p>
            </div>
            ...
            <div className="relative group cursor-pointer flex flex-col justify-center items-center w-40 h-auto bg-neutral-900 text-gray-50 rounded-2xl shadow-lg overflow-hidden">
              <div className="after:duration-500 before:duration-500 duration-500 
                ...
                <span className=''>{String(formattedHour).padStart(2, '0')}</span>
    :
    <span className=''>{String(currentMinute).padStart(2, '0')}</span>
    <span className="text-center text-sm mt-3">{amPm}</span>
                </div>
              </div>
