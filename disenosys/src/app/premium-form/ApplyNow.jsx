...
const ApplyNow = () => {
  ...
  const diffTime = Math.max(0, 30 - Math.floor((today - createdDate) / (1000 * 60 * 60 * 24)));
  ...
  useEffect(() => {
    ...
    const findEmail = payment
      .filter((data) => data.sessionId === sessionId)
      .map((data) => data.customerDetails.email);
      const createJobDate = payment
      .filter((data) => data.sessionId === sessionId)
      .map((data) => data.createdAt);
    setGetDate(createJobDate[0]);
    setFormData((prev) => ({
      ...prev,
      name: findName[0],
      email: findEmail[0],
  ...
  return (
    <div className="bg-[#FBFBFB] min-h-screen font-garet">
      {payment
        ?.filter((data, i) => data.sessionId === sessionId)
        .map(
          (data, i) =>
            data.isPaid && data.appliedCount <= 5 &&  (
              <div className="container mx-auto" key={i}>
                <div className="flex flex-col lg:flex-row justify-between items-center py-3 border-b-2 border-gray-200">
                  ...
                  <h4 className="font-semibold text-sm lg:text-lg text-gray-700 flex items-center gap-2">
                    Welcome - {data.customerDetails.name}
                    <FiUserCheck size={20} color="blue" />
                  </h4>
                </div>
                 
                <p className="text-lg font-bold text-white bg-red-500 text-center p-2 rounded-sm mt-2 shadow-inner">
                  Total Applications Used : {" "}
                  <span className="font-bold">
                    {data.appliedCount} of 5
                  </span>
                </p>

                <p className="text-sm text-gray-700 mt-1">
                  <strong>Subscription Started:</strong> {new Date(data.createdAt).toLocaleDateString()}           
                </p>
                <p className="text-sm text-gray-700 flex gap-2 items-center ">
                 <strong>Days Remaining:</strong> {diffTime} day{diffTime !== 1 ? "s" : ""} left
                 <span className="w-2 h-2 animate-pulse bg-red-500 ring-2 ring-blue-100 shadow-xl rounded-full"></span>
                </p>

                <form
                  onSubmit={handleSubmit}
                  className="grid grid-cols-1 md:grid-cols-2 gap-8 rounded-3xl shadow-inner p-10 mt-8"
                >
                  <div className="flex flex-col gap-4">
                    ...
                    <input
                      ...
                      required
                      placeholder={data.customerDetails.name}
                      disabled
                      className="border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-not-allowed"
                    />
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    <input
                      ...
                      onChange={handleChange}
                      ref={fileInputRef}
                      className="border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 p-1file:mr-4 file:py-2 file:px-4 file:rounded file:border file:border-gray-300 file:text-sm file:font-medium file:bg-gray-100 hover:file:bg-gray-200 "
                      required
                    />
                  ...
                  <div className="col-span-1 md:col-span-2 text-center mt-4">
                    <button
                      type="submit"
                      className="bg-blue-500 hover:bg-blue-700 text-white px-8 py-3 font-medium rounded-xl shadow-md"
                    >
                      Submit
                    </button>
                  </div>
