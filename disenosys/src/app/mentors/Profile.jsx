...
const Profile = () => {
    ...
    const applyFilters = () => {
      ...
      const filtered = data?.data?.filter((profile) => {
        const profileExperience = profile.exp || "";
        const component = profile.automotive || "";
  
        const matchesExperience = exp
          ? profileExperience.includes(exp)
          : true;
  
        const matchesComponent = automotive ? component.includes(automotive) : true;
          
        return (
          matchesExperience && matchesComponent 
        );
      });
